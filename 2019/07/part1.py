...
# input
def setInput(lines, index, userinput=''):
    if userinput == '':
...
def main(lines, inputs=[]):
    ...
    while index < len(lines):
        ...
        if opcode == 1:
            ...
            if len(inputs) > 0:
                lines = setInput(lines, index, inputs.pop())
            else:
...
def permutations_list(charsequence):
    return [''.join(p) for p in permutations(charsequence)]

def thruster_signal(signal, lines):
    start = '0'
    out = ""
    for char in signal:
        logging.warning([char, start])
        start = main(lines, [char, start])
...
def test01():
    inputfile = "p1_test1.txt"
    lines = open(inputfile, "r").read().split(',')
    perms = permutations_list('01234')
    diagnostic_list = []
    print(perms)
    print()
    print(lines)
    ...
    for str in perms:
        logging.warning(str)
        diagnostic = ""
        diagnostic = thruster_signal(str, lines)
        print("diagnostic code: " + diagnostic)
        diagnostic_list.append(diagnostic)
    logging.warning(diagnostic_list)
